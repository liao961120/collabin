...
1. 在處理停用字表的時候發現，即使檔案命名為 .txt ，還是有無法運作的狀況。我想到之前在 Lope 的 python 的讀書會中有提過這樣的情況，後來我打開編輯器重新當上一次停用字表內容並存檔，就解決了這個問題。    

2. 因為讀檔後的資料無法進行斷詞，所以回頭去挖它的 mode() ，後來就發現了 str() 這個函數，可以直接去看物件的結構組成，覺得很方便。  
